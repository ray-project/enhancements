...
3. Deprecate the following `DatasetConfig` arguments when streaming is enabled: `max_object_store_memory_fraction`, `global_shuffle`, `fit`, transform`, `randomize\_block\_order`, and `per\_epoch\_preprocessor`, as well as the following `Trainer` arguments: `preprocessor`.
...
# Per-epoch preprocessing (note: lazy).
train_ds = train_ds \
