...
# Create the per-epoch preprocessor, and flag it for train only.
...
- A: The checkpoint will have the full chain, including per-epoch ones. Preprocessors can be tagged as used for training only / ignored during inference by setting an `ignore_for_inference` attribute on them.
